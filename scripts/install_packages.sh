...
function install_puppet(){
	...
	if [ -f ${tmp_file} ]; then
		...
		sudo apt-get install -y ${puppet_binary}-common=${puppet_version} $puppet_binary=${puppet_version}
		# Detect the current version using the puppet binary
...
function install_jenkins(){
	...
	[ $? -ne 0 ] && echo "Unexpected error occured during installation!" && return 1
